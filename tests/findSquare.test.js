...
test("empty squares should return no square (null)", () => {
  expect(GameEngineChipMoves.findSquare([], testConstants.chip)).toBe(undefined);
});

test("squares=null should return no square (null)", () => {
  expect(GameEngineChipMoves.findSquare(null, testConstants.chip)).toBe(undefined);
});

test("chip on [1,1] should return square on [1,1]", () => {
  testConstants.squares[0].activeChip = testConstants.chip;
  expect(GameEngineChipMoves.findSquare(testConstants.squares, testConstants.chip)).toBe(testConstants.squares[0]);
});
